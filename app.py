...
# Note: Using .get() ensures it doesn't crash if the variable isn't created yet
...
with tab2:
    ...
    if annual_files:
        st.session_state['h15_complete'] = True # Add this
        if st.button("🚀 Process & Generate HMDAS-15"):
            try:
                # 1. Initialize logic
                extracted_data = []
                detected_stn = ""
                ...
                # Load the internal template once
                wb_h15 = load_workbook(HMDAS15_TEMPLATE_PATH, keep_vba=True)
                ws_summary = wb_h15.active
                
                for uploaded_file in annual_files:
                    # 1. Extract Station Name from Filename
                    fname = uploaded_file.name
                    parts = fname.replace("-", " ").replace("_", " ").split()
                    if len(parts) >= 3:
                        detected_stn = parts[2]
                    
                    # 2. Load Workbook (Use default mode, not data_only, to see all cells)
                    ...
                    for sheet_name in wb_h14.sheetnames:
                        if date_pattern.match(sheet_name):
                            ...
                            # --- MANUAL MAX CALCULATION FOR COLUMNS C TO L ---
                            daily_max_values = []
                            
                            # Column 3 = C (10min), Column 12 = L (Daily)
                            for col_idx in range(3, 13): 
                                vals_in_col = []
                                for row_idx in range(4, 148): # Rows 4 to 147 hold the actual data
                                    cell_val = ws_day.cell(row=row_idx, column=col_idx).value
                                    
                                    # Clean and validate the number
                                    if cell_val is None or str(cell_val).strip().upper() == 'T':
                                        vals_in_col.append(0.0)
                                    else:
                                        try:
                                            vals_in_col.append(float(cell_val))
                                        except (ValueError, TypeError):
                                            vals_in_col.append(0.0)
                                
                                # Find the Max for this specific duration column
                                col_max = max(vals_in_col) if vals_in_col else 0.0
                                daily_max_values.append(col_max)
                            
                            extracted_data.append({
                                "date_label": sheet_name,
                                "values": daily_max_values
                            })
                            
                            if not detected_year:
                                detected_year = sheet_name.split()[-1]

                # --- Sorting & Injection remains the same ---
                extracted_data.sort(key=lambda x: datetime.strptime(x['date_label'], "%d %b %Y"))

                for idx, entry in enumerate(extracted_data, 1):
                    row_idx = 3 + idx
                    ws_summary.cell(row=row_idx, column=1).value = idx
                    ws_summary.cell(row=row_idx, column=2).value = entry['date_label']
                    
                    for c_idx, val in enumerate(entry['values']):
                        # PASTE AS NUMBERS
                        ws_summary.cell(row=row_idx, column=3 + c_idx).value = val
                
                # 4. Final Branding
                ws_summary["A2"] = f"{detected_stn} Synop Station"
                new_sheet_name = f"{detected_year} {detected_stn} Max Summary"
                ws_summary.title = new_sheet_name

                # 5. Output
                output_fn = f"HMDAS-15 {detected_stn} {detected_year}.xlsm"
                h15_buffer = io.BytesIO()
                wb_h15.save(h15_buffer)
                
                st.success(f"✅ Extracted data for **{detected_stn}** ({detected_year})")
                st.download_button(
